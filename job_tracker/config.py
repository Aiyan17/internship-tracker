...
@dataclass
class Settings:
    ntfy_topic: str = "my_internship_alerts"
    email: str = ""
...
def load_config(config_path: str = "config.yaml") -> AppConfig:
    ...
    topic = os.environ.get("NTFY_TOPIC") or s_data.get("ntfy_topic") or "my_internship_alerts"
    ...
    settings = Settings(
        ...
        db_path=s_data.get("db_path", "job_state.db"),
    )

    f_data = data.get("filters", {})
    filters = Filters(
        role_keywords=[k.lower() for k in f_data.get("role_keywords", [])],
        topic_keywords=[k.lower() for k in f_data.get("topic_keywords", [])],
        location_include=[k.lower() for k in f_data.get("location_include", [])],
        visa_flag_keywords=[k.lower() for k in f_data.get("visa_flag_keywords", [])],
    )
